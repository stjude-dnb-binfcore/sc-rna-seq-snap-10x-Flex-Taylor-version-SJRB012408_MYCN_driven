...
This notebook is relevant to projects in which we need to remove cell contamination from the object, e.g., clusters. After inspection of the two rounds of results in the `cluster-cell-calling` module, the user can remove clusters if necessary. For example, in PDX projects there may be both human and mouse clusters identified after the `02-find-markers.Rmd` step in `cluster-cell-calling`. We then recommend running the `cell-contamination-removal-analysis` module to remove contaminating clusters and repeat normalization and integration. This object can then be used for cell type annotation or other types of analysis (see `README.md` in this folder, documentation in the `yaml` file, and root `README.md`).
...
First, we use the Seurat object from `01-cluster-cell-calling.Rmd` in `cluster-cell-calling` (`seurat_obj_clusters_all.rds` under results `01_cluster_cell_calling_{resolution_cluster_folder}`). Clusters used for contamination removal use the granularity as defined in params as `resolution_list` (typically the same resolution chosen for `02-find-markers.Rmd` in the `cluster-cell-calling` module, defined in `project_parameters.Config.yaml`).
...
# Remove previous clustering, UMAP duplicates and PC-related columns from metadata
seurat_obj$seurat_clusters <- NULL
seurat_obj@meta.data <- seurat_obj@meta.data %>% 
  select(-contains(c("UMAPndim", "PC", "snn_res.")),
         -matches("UMAP_.*\\.[xy]$")) %>%  # removes UMAP_1.x, UMAP_2.x, UMAP_1.y, UMAP_2.y
  distinct()
...
# Find columns related to UMAP
#umap_cols <- grep("umap", colnames(seurat_obj@meta.data), value = TRUE, ignore.case = TRUE)

# If found, print them
#if (length(umap_cols) > 0) {
#  print(umap_cols)
#} else {
#  message("No UMAP columns found in meta.data")
#}
##############################################################################################
